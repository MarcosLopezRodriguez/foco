...
const useAppStore = create<AppState>((set, get) => ({
  ...
  completeTask: (id) => {
    ...
    if (!t) return;
    const patch: Partial<Task> = { estado: "hecha", snoozeUntil: null };
  ...
  clearDoneToday: () => {
    ...
    set((s) => {
      const keep = s.tasks.filter((t) => !(t.estado === "hecha" && parseISO(t.createdAt) >= today));
      db.transaction("rw", db.table("tasks"), async () => {
        await db.table("tasks").clear();
...
type View = "focus" | "triage" | "stats" | "settings";
...
function StatsView() {
  ...
  const doneToday = tasks.filter((t) => t.estado === "hecha" && isAfter(parseISO(t.createdAt), startOfDay(today)));
